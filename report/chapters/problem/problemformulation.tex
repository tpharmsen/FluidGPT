...
\begin{enumerate}
	\item question about comparison on single next timeblock ar vs fm
	\item question about question about rollout 
	\item question about physics correctness
	\item idea: deterministic averages, probailistic conserves fallout of vortices -> 
	
	\item Can we develop a deterministic baseline that generalises across diverse Navier-Stokes scenarios using only velocity-field inputs, trained on large-scale multi-source pretraining data?
	\item Can a probabilistic approach yield better results than the deterministic benchmark in the scope of:
	\subitem Single next time-block prediction
	\subitem Rollout prediction
	\subitem Energy and Enstrophy spectra statistics
