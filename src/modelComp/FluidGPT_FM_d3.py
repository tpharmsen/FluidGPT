...
class FluidGPT_FM(nn.Module):
    def __init__(self, emb_dim=96, embedder_type="linear", data_dim=[64,3,2,128,128], patch_size=(8,8), hiddenout_dim=128, flowmatching_emb_dim=256,
        ...
        self.flow_d1 = nn.Linear(flowmatching_emb_dim, 16)
        self.flow_d2 = nn.Linear(flowmatching_emb_dim, 32)
        self.flow_d3 = nn.Linear(flowmatching_emb_dim, 64)
        self.flow_d4 = nn.Linear(flowmatching_emb_dim, 128)
        self.flow_d5 = nn.Linear(flowmatching_emb_dim, 64)
        self.flow_d6 = nn.Linear(flowmatching_emb_dim, 32)
        self.flow_d7 = nn.Linear(flowmatching_emb_dim, 16)
        ...
        self.enable_final_layer = enable_final_layer
        ...
        for i in range(depth):
            ...
            for j in range(stage_depths[i]):
                ...
                if j % 3 == 0:
                    self.resnorms_down[i].append(nn.LayerNorm((emb_dim + 16) * 2**i))
        ...
        for i in range(stage_depths[depth]):
            ...
            if i % 2 == 0:
                self.resnorms_middle.append(nn.LayerNorm((emb_dim + 16) * 2**depth))
        ...
        for i in reversed(range(depth)):
            ...
            for j in range(stage_depths[depth + i + 1]):
                ...
                if j % 3 == 0:
                    self.resnorms_up[depth - i - 1].append(nn.LayerNorm((emb_dim + 16) * 2**i))
    ...
    def forward(self, x, t, extra={None}):
        ...
        t4 = self.flow_d4(t4)
        t4 = t4.unsqueeze(1).unsqueeze(2).repeat(1, x.shape[1], x.shape[2] // 64, 1)

        t5 = self.flowt_act(t0)
        t5 = self.flow_d5(t5)
        t5 = t5.unsqueeze(1).unsqueeze(2).repeat(1, x.shape[1], x.shape[2] // 16, 1)
        
        t6 = self.flowt_act(t0)
        t6 = self.flow_d6(t6)
        t6 = t6.unsqueeze(1).unsqueeze(2).repeat(1, x.shape[1], x.shape[2] // 4, 1)

        t7 = self.flowt_act(t0)
        t7 = self.flow_d7(t7)
        t7 = t7.unsqueeze(1).unsqueeze(2).repeat(1, x.shape[1], x.shape[2], 1)
            ...
            for j, module in enumerate(module_list):
                if j % 2 == 0:
                    ...
                    if i == 0:
                        ...
                        x = torch.concat((x, t6), dim=3)
                    else:
                        #x = x + t5
                        x = torch.concat((x, t7), dim=3)
                    x = self.resnorms_up[i][j // 2](x)
                x = module(x)
                if j % 2 == 1:
                    x = x[:,:,:,:(-16 * 2**(self.depth - i - 1))]
                    if residual is not None:
                        x = x + residual
